...
function tokenizer(state, startLine, endLine, silent) {
  ...
  // Match "youtube/autoplay"
  let matchServiceName = serviceName.match(/(\w+)(\/(\w+))?/i);

  // Lookup service; if unknown, then this is not a known embed!
  let service = this.services[matchServiceName[1]];
  ...
  if (!silent) {
    ...
    token.block = true;
    token.info = {
      serviceName: matchServiceName[1],
      service: service,
      videoReference: videoReference,
      videoID: service.extractVideoID(videoReference),
      autoplay: matchServiceName[3] === 'autoplay'
    };
